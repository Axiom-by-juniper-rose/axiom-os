...
Builds depreciation schedules per IRS rules including bonus depreciation taper.
"""
import logging
...
# IRS MACRS useful lives by asset class
MACRS_LIVES: dict[str, float] = {
    "residential":          27.5,
    "commercial":           39.0,
...
# Bonus depreciation phase-down (TCJA)
BONUS_RATES: dict[int, float] = {
    2022: 1.00,
    2023: 0.80,
    2024: 0.60,
    2025: 0.40,
...
def get_bonus_rate(year: int) -> float:
    """Return applicable bonus depreciation % for given tax year."""
...
def build_macrs_schedule(
    ...
    method: Literal["MACRS", "SL", "bonus"] = "MACRS",
) -> list[dict]:
    """
    Generate a year-by-year depreciation schedule.

    Returns list of dicts: [{year, deduction, remaining_basis, method}]
    """
    ...
    if method == "bonus":
        ...
        if bonus_rate > 0:
            first_year_deduction = cost_basis * bonus_rate
            remaining -= first_year_deduction
            schedule.append({
                "year": year,
                "deduction": round(first_year_deduction, 2),
                "remaining_basis": round(remaining, 2),
                "method": f"bonus_{int(bonus_rate * 100)}pct",
            })
            year += 1
            cost_basis = remaining
        # Continue with SL on remaining basis
        method = "SL"
        useful_life = max(useful_life - 1, 1)

    # Straight-line on remaining basis
    if useful_life <= 0:
        return schedule

    annual_deduction = cost_basis / useful_life
    periods = int(useful_life) + (1 if useful_life % 1 > 0 else 0)

    for i in range(periods):
        if remaining <= 0.01:
            break
        deduction = min(annual_deduction, remaining)
...
def save_schedule(project_id: str, asset_class: str, cost_basis: float,
                  placed_in_service_date: date, method: str,
                  schedule: list[dict], supabase) -> Optional[str]:
    """Persist depreciation schedule to Supabase."""
