 ...
 * Real-time visual display of the 8-agent pipeline.
...
export function AgentHandoff({ dealId, onComplete }: Props) {
  ...
  return (
    <div style={{
      ...
      borderRadius: 12, padding: 24, fontFamily: 'Instrument Sans, sans-serif',
    }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 800, color: C.text, fontSize: 16, margin: 0 }}>
          Agent Pipeline
        </h3>
        <span style={{ fontFamily: 'DM Mono, monospace', color: C.textMid, fontSize: 12 }}>
        ...
        {tasks.map((task) => {
          ...
          return (
            <div key={task.id} style={{
              ...
              border: `1px solid ${task.status === 'completed' ? 'rgba(74,222,128,0.15)' : C.border}`,
              ...
              <span style={{ flex: 1, color: C.text, fontSize: 13 }}>
                {task.label}
              </span>
              <span style={{ fontFamily: 'DM Mono, monospace', color: C.textMid, fontSize: 11 }}>
                {task.status}
              </span>
            </div>
          );
