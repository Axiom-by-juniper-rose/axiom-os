...
function Agent({ id, system, placeholder }) {
  ...
  const [model, setModel] = useLS("axiom_agent_model", "claude-sonnet-4-20250514");
          ...
          <div style={{ gridColumn: layout === "Compact" ? "1/3" : "1/3" }}>
            <Card title="Projected Cash Flow ($ Millions)">
              <ResponsiveContainer width="100%" height={220}>
                <AreaChart data={cfData} onClick={e => { if (e && e.activePayload && e.activePayload[0]) setChartSel(e.activePayload[0]); }} style={{ cursor: 'pointer' }}>
      ...
      <div style={layout === "Compact" ? S.g3 : S.g2}>
        <Card title="Cost Allocation">
          <ResponsiveContainer width="100%" height={210}>
            <PieChart onClick={e => { if (e && e.activePayload && e.activePayload[0]) setChartSel(e.activePayload[0]); }} style={{ cursor: 'pointer' }}>
        ...
        <Card title="Deal Readiness Radar">
          <ResponsiveContainer width="100%" height={210}>
            <RadarChart data={radarD} onClick={e => { if (e && e.activePayload && e.activePayload[0]) setChartSel(e.activePayload[0]); }} style={{ cursor: 'pointer' }}>
      ...
      <div>
        <Card title="Price Trend Analysis">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={chartD} onClick={e => { if (e && e.activePayload && e.activePayload[0]) setChartSel(e.activePayload[0]); }} style={{ cursor: 'pointer' }}>
...
function FinancialEngine() {
  ...
  return (
    <Tabs tabs={["Pro Forma", "Calculators", "Sensitivity", "Loan & Equity"]}>
      ...
      <div>
        <Card title="Construction Loan Parameters">
          <div style={S.g3}>
            {[["LTC % (Loan-to-Cost)", "70%"], ["Interest Rate %", "9.5%"], ["Loan Term (months)", "24"], ["Extension Option (months)", "12"], ["Origination Fee %", "1.5%"], ["Target Lender", ""]].map(([l, ph], i) => (
              <Field key={i} label={l}><input style={S.inp} placeholder={ph} /></Field>
            ))}
          </div>
          <Field label="Lender Notes / Conditions"><textarea style={{ ...S.ta, height: 55 }} placeholder="Draw schedule, reporting requirements, recourse terms..." /></Field>
        </Card>
        <Card title="Equity Structure">
          <div style={S.g3}>
            {[["GP Equity %", "10"], ["LP Equity %", "90"], ["Preferred Return % (LP)", "8"], ["Promote / Carry %", "20"], ["Equity Multiple Target", "2.0x"], ["IRR Target", "18%"]].map(([l, ph], i) => (
              <Field key={i} label={l}><input style={S.inp} placeholder={ph} /></Field>
            ))}
          </div>
        </Card>
        ...
        <Card title="Risk by Category (Open Risks)">
          <ResponsiveContainer width="100%" height={220}>
            <RadarChart data={matrixData}>
              ...
              <Radar dataKey="A" stroke={C.red} fill={C.red} fillOpacity={0.2} strokeWidth={1.5} />
            </RadarChart>
          </ResponsiveContainer>
        </Card>
        <Card title="Severity Distribution">
          <ResponsiveContainer width="100%" height={210}>
            <BarChart data={["Critical", "High", "Medium", "Low"].map(s => ({ name: s, count: risks.filter(r => r.severity === s).length, fill: RC[s] }))} onClick={e => { if (e && e.activePayload) setChartSel(e.activePayload[0].payload); }} style={{ cursor: 'pointer' }}>
              <CartesianGrid strokeDasharray="3 6" stroke={C.border} strokeOpacity={0.5} vertical={false} />
              <XAxis dataKey="name" stroke={C.dim} tick={{ fontSize: 13, fontFamily: 'Inter,sans-serif', fill: C.muted }} />
...
function ReportsBinder() {
  const { fin, project, risks } = usePrj();
  const hard = fin.totalLots * fin.hardCostPerLot, soft = hard * fin.softCostPct / 100;
  const fees = fin.planningFees + (fin.permitFeePerLot + fin.schoolFee + fin.impactFeePerLot) * fin.totalLots;
  const cont = (hard + soft) * fin.contingencyPct / 100;
  const totalCost = fin.landCost + fin.closingCosts + hard + soft + cont + fees;
  ...
  return (
    <Tabs tabs={["Binder Contents", "Generate Report", "Export Options"]}>
      <div>
      ...
      <div>
        <Card title="AI Report Generator">
          <div style={{ background: C.bg, border: `1px solid ${C.border}`, borderRadius: 4, padding: 16, marginBottom: 14 }}>
            <div style={{ fontSize: 10, color: C.dim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Project Snapshot</div>
            <div style={S.g3}>
              {[["Lots", fin.totalLots], ["Revenue", fmt.M(revenue)], ["Profit", fmt.M(profit)], ["Margin", fmt.pct(margin)], ["Risks", risks.filter(r => r.status === "Open").length + " open"], ["Status", project.name ? "Active" : "—"]].map(([l, v]) => (
                <div key={l}><div style={{ fontSize: 9, color: C.dim }}>{l}</div><div style={{ fontSize: 15, color: C.text, fontWeight: 600 }}>{v}</div></div>
              ))}
            </div>
          </div>
          <Agent id="ReportGenerator" system="You are a real estate development report writer producing investor-grade feasibility analysis documents. Generate comprehensive, data-rich sections including executive summaries, financial analyses, market analyses, and risk assessments." placeholder="Type 'generate executive summary' or 'write market analysis section' to begin..." />
        </Card>
      </div>
      <div>
          ...
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: "1px solid #0F1117" }}>
              ...
              <button style={S.btn("gold")} onClick={() => {
                if (e.type === "csv") {
                  downloadCSV(["Section", "Data"], sections.map(s => [s, "Feasibility Data Placeholder"]), `axiom_${e.fmt.split(" ")[0].toLowerCase()}${e.ext}`);
                } else {
                  downloadText(`AXIOM OS - ${e.fmt}\nProject: ${project.name || "Unnamed"}\n\nGenerated: ${new Date().toLocaleString()}\n\nFull development binder and feasibility analysis details here.`, `axiom_${e.fmt.split(" ")[0].toLowerCase()}${e.ext}`);
                }
              }}>Export</button>
            </div>
          ))}
...
function DealAnalyzer() {
  ...
  return (
    <Tabs tabs={["Quick Analysis", "Deep Dive", "Go/No-Go", "IC Memo Builder"]}>
      ...
      <div>
        <Card title="Investment Committee Memo Builder">
    ...
    <Tabs tabs={["Intel Feed", "Add Record", "Analytics", "Live Market"]}>
      ...
      <div>
        <Card title="Intel by Category">
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={IT.map(t => ({ name: t.split(" ")[0], count: records.filter(r => r.type === t).length }))} onClick={e => { if (e && e.activePayload && e.activePayload[0]) setChartSel(e.activePayload[0]); }} style={{ cursor: 'pointer' }}>
              <CartesianGrid strokeDasharray="3 6" stroke={C.border} strokeOpacity={0.5} vertical={false} />
              <XAxis dataKey="name" stroke={C.dim} tick={{ fontSize: 11, fontFamily: 'Inter,sans-serif', fill: C.muted }} />
              <YAxis stroke={C.dim} tick={{ fontSize: 11, fontFamily: 'Inter,sans-serif', fill: C.muted }} allowDecimals={false} />
              <Tooltip separator=" " contentStyle={{ background: "rgba(17,19,24,0.95)", border: `1px solid ${C.gold}33`, borderRadius: 8, fontSize: 13, padding: "10px 14px", boxShadow: "0 8px 32px rgba(212,168,67,0.15)" }} cursor={{ fill: C.bg4 }} />
...
function Agents() {
  return (
    <div style={S.g3}>
      <Card title="Acquisition AI"><Agent id="AcqAgent" system="You are an acquisition analyst." placeholder="Ask about deals..." /></Card>
      <Card title="Legal AI"><Agent id="LegalAgent" system="You are a real estate attorney." placeholder="Upload contracts..." /></Card>
      <Card title="Finance AI"><Agent id="FinAgent" system="You are a real estate CFO." placeholder="Analyze cash flow..." /></Card>
    </div>
  );
}
    ...
    <Tabs tabs={["Vendor Directory", "Compliance Tracking", "Bid Management"]}>
      ...
      <div>
        <Card title="Active RFPs & Bid Management">
          <table style={S.tbl}>
            <thead><tr><th style={S.th}>Trade / Package</th><th style={S.th}>Bidders Invited</th><th style={S.th}>Bids Received</th><th style={S.th}>Current Low Bid</th><th style={S.th}>Status</th></tr></thead>
            <tbody>
              <tr><td style={{ ...S.td, color: C.text }}>Earthwork & Grading</td><td style={S.td}>4</td><td style={S.td}>3</td><td style={{ ...S.td, color: C.gold }}>$1,240,000</td><td style={S.td}><Badge label="Leveling" color={C.amber} /></td></tr>
              <tr><td style={{ ...S.td, color: C.text }}>Wet Utilities</td><td style={S.td}>3</td><td style={S.td}>0</td><td style={{ ...S.td, color: C.dim }}>--</td><td style={S.td}><Badge label="Bidding" color={C.blue} /></td></tr>
              <tr><td style={{ ...S.td, color: C.text }}>Site Concrete</td><td style={S.td}>5</td><td style={S.td}>5</td><td style={{ ...S.td, color: C.gold }}>$485,000</td><td style={S.td}><Badge label="Awarded" color={C.green} /></td></tr>
            </tbody>
          </table>
          <button style={{ ...S.btn("gold"), marginTop: 16 }}>Create New RFP</button>
        </Card>
      </div>
    </Tabs>
  );
...
function BillingPlans() {
  ...
  const tiers = [
    { id: "free", name: "Free", price: 0, desc: "For individuals exploring the market.", features: ["5 Active Deals", "Basic Mortgage Calculator", "Public Data Access", "Community Support"], notIncluded: ["Export to PDF/CSV", "Advanced Calculators", "Intel Record Linking", "MLS Integration", "AI Agent Hub", "Priority Support"] },
    { id: "pro", name: "Pro", price: 29, desc: "For serious investors building a portfolio.", features: ["Unlimited Deals", "All 9 Calculators", "PDF & CSV Exports", "Intel Record Linking", "MLS & Listing Feeds", "Email Support", "5 AI Agent Sessions/day", "Saved Searches (10)"], notIncluded: ["Team Collaboration", "API Access", "Custom Branding"], recommended: true },
    { id: "pro_plus", name: "Pro+", price: 99, desc: "For agencies and teams scaling up.", features: ["Everything in Pro", "Team Collaboration (5 seats)", "API Access", "Unlimited AI Agent Sessions", "Unlimited Saved Searches", "Dedicated Account Manager", "Custom Branding", "White-Label Reports", "Priority Phone Support"], notIncluded: [] },
    { id: "enterprise", name: "Enterprise", price: 299, desc: "For development firms and institutional investors.", features: ["Everything in Pro+", "Unlimited Team Seats", "Custom Integrations", "On-Premise Deployment Option", "SLA Guarantee (99.9%)", "Custom AI Model Training", "Dedicated Success Manager", "Quarterly Business Reviews"], notIncluded: [] },
  ];
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14 }}>
        {tiers.map(t => (
          <div key={t.id} style={{ background: C.bg3, border: `1px solid ${t.recommended ? C.gold : C.border}`, borderRadius: 4, padding: 20, position: "relative", display: "flex", flexDirection: "column" }}>
            {t.recommended && <div style={{ position: "absolute", top: -8, right: 12, ...S.tag(C.gold), fontSize: 8 }}>MOST POPULAR</div>}
            ...
            <button style={{ ...S.btn(current === t.id ? "" : "gold"), marginTop: 14, width: "100%", opacity: current === t.id ? 0.5 : 1 }} onClick={() => setCurrent(t.id)} disabled={current === t.id}>
              {current === t.id ? "Current Plan" : `Upgrade to ${t.name}`}
            </button>
            <div style={{ marginTop: 14, flex: 1 }}>
              <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 8 }}>Included</div>
              {t.features.map((f, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0" }}>
                  <span style={{ color: C.green, fontSize: 12 }}>✓</span>
                  <span style={{ fontSize: 10, color: C.sub }}>{f}</span>
                </div>
              ))}
              {t.notIncluded.map((f, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: 6, padding: "3px 0", opacity: 0.4 }}>
                  <span style={{ color: C.muted, fontSize: 12 }}>×</span>
                  <span style={{ fontSize: 10, color: C.muted, textDecoration: "line-through" }}>{f}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <Card title="Subscription Management">
        <div style={S.g2}>
          <div>
            <div style={{ fontSize: 9, color: C.dim, letterSpacing: 2, textTransform: "uppercase", marginBottom: 6 }}>Current Plan</div>
            <div style={{ fontSize: 21, color: C.gold, fontWeight: 700, textTransform: "uppercase" }}>{current}</div>
            <div style={{ fontSize: 12, color: C.dim, marginTop: 4 }}>Billing cycle: Monthly · Next renewal: March 22, 2025</div>
          </div>
          <div style={{ display: "flex", gap: 8, alignItems: "center", justifyContent: "flex-end" }}>
            <button style={S.btn()}>View Invoice History</button>
            <button style={S.btn()}>Update Payment</button>
            <button style={{ ...S.btn(), color: C.red, borderColor: C.red }}>Cancel Plan</button>
          </div>
        </div>
      </Card>
    </div>
    ...
    <Tabs tabs={["Profile", "API Keys", "Notifications", "Team", "Data"]}>
      ...
      <div>
        <Card title="API Key Management">
          <div style={{ fontSize: 12, color: C.dim, marginBottom: 14 }}>API keys are stored locally and never sent to our servers. They are used to power AI features and data integrations.</div>
      ...
      <div>
        <Card title="Team Management">
          <div style={{ fontSize: 12, color: C.dim, marginBottom: 14 }}>Invite team members to collaborate on deals and projects.</div>
          {[["Admin", "Full access to all features, billing, and team management"], ["Analyst", "View and edit deals, run calculators, access data layers"], ["Viewer", "Read-only access to deals and reports"]].map(([role, desc], i) => (
            <div key={i} style={{ padding: "10px 0", borderBottom: "1px solid #0F1117" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div><div style={{ fontSize: 13, color: C.text, fontWeight: 600 }}>{role}</div><div style={{ fontSize: 10, color: C.dim }}>{desc}</div></div>
                <button style={{ ...S.btn(), padding: "4px 10px", fontSize: 9 }} onClick={() => doSave("inv-" + role)}>{saved === "inv-" + role ? "✓ Invited!" : "Invite"}</button>
              </div>
            </div>
          ))}
          <div style={{ marginTop: 14 }}><Field label="Invite by Email"><div style={{ display: "flex", gap: 8 }}><input style={{ ...S.inp, flex: 1 }} placeholder="colleague@company.com" /><button style={S.btn("gold")} onClick={() => doSave("email-inv")}>{saved === "email-inv" ? "✓ Invite Sent!" : "Send Invite"}</button></div></Field></div>
        </Card>
      </div>
      <div>
...
function Notes() {
  const [notes, setNotes] = useLS("axiom_notes", [
    { id: 1, title: "Sunset Ridge - Due Diligence Checklist", content: "Phase I ESA: Clean (received 02/10)\nGeotech: Pending - West Valley Labs\nAPNs: 123-456-789, 123-456-790\nTitle: Preliminary report ordered from Chicago Title\nSurvey: ALTA survey in progress (ETA 02/28)\nWetlands: None identified per NWI mapping\nFlood: Zone X confirmed via FEMA panel 06065C0720G", deal: "Sunset Ridge", category: "Due Diligence", pinned: true, created: "2025-02-10", modified: "2025-02-20" },
    { id: 2, title: "IC Meeting Notes - Feb 14", content: "Attendees: Sarah, Mike, David, Jennifer\n\nKey decisions:\n1. Proceed with Sunset Ridge LOI at $3.0M (approved)\n2. Hawk Valley - need updated comps before committee review\n3. Budget approved for Phase I ESA on Meadowbrook\n\nAction items:\n- Sarah: Draft LOI by Feb 16\n- Mike: Pull comp data for Hawk Valley\n- David: Schedule Meadowbrook site visit", deal: "General", category: "Meeting Notes", pinned: false, created: "2025-02-14", modified: "2025-02-14" },
    { id: 3, title: "Market Analysis - Sacramento Infill", content: "Key findings from Q4 2024:\n- Finished lot prices: $165K-$195K (up 8% YoY)\n- Absorption: 3.2 lots/month avg for 40-60 lot subdivisions\n- New competition: 3 projects in entitlement within 5-mile radius\n- Builder demand: Strong from national builders (Lennar, DR Horton)\n- Land supply: Tightening, especially entitled parcels\n\nImplications for pricing strategy:\n- Target $178K-$185K per lot\n- 14-month sell-out timeline is achievable", deal: "General", category: "Research", pinned: true, created: "2025-01-20", modified: "2025-02-05" },
  ]);
    ...
    <Tabs tabs={["All Notes", "New Note", "AI Summary"]}>
          ...
          <Card key={n.id} title={n.title} action={<div style={{ display: "flex", gap: 6 }}>{n.pinned && <Badge label="Pinned" color={C.gold} />}<Badge label={n.category} color={C.blue} /><button style={{ ...S.btn(), padding: "3px 8px", fontSize: 9 }} onClick={() => setEditing(editing === n.id ? null : n.id)}>{editing === n.id ? "Close" : "Edit"}</button><button style={{ ...S.btn(), padding: "3px 8px", fontSize: 9 }} onClick={() => updNote(n.id, "pinned", !n.pinned)}>{n.pinned ? "Unpin" : "Pin"}</button><button style={{ ...S.btn(), padding: "3px 8px", fontSize: 9 }} onClick={() => delNote(n.id)}>x</button></div>}>
            {editing === n.id ? (
              <div>
                ...
                <Field label="Linked Deal"><input style={S.inp} value={n.deal} onChange={e => updNote(n.id, "deal", e.target.value)} /></Field>
      ...
      <div>
        <Card title="Create New Note">
          ...
          <Field label="Linked Deal (optional)"><input style={S.inp} value={nn.deal} onChange={e => setNn({ ...nn, deal: e.target.value })} placeholder="e.g. Sunset Ridge" /></Field>
          <Field label="Content"><textarea style={{ ...S.ta, height: 200, fontFamily: "'Courier New',monospace", fontSize: 13 }} value={nn.content} onChange={e => setNn({ ...nn, content: e.target.value })} placeholder="Write your note here... (supports plain text and markdown-style formatting)" /></Field>
          <button style={S.btn("gold")} onClick={addNote}>Save Note</button>
        </Card>
      </div>
      <div>
        <Card title="AI Note Summary">
          <Agent id="NotesAI" system={`You are a note-taking assistant for a real estate developer. The user has ${notes.length} notes across categories. Help summarize, organize, and extract action items from notes. Current notes: ${notes.map(n => n.title).join(", ")}.`} placeholder="Ask to summarize notes, extract action items, or find related info..." />
        </Card>
      </div>
    </Tabs>
  );
...
function NeuralNet() {
  ...
  const runPrediction = async () => {
    ...
    setPredictions({
      lotPrice: { low: 140000, mid: 165000, high: 195000, conf: 87 },
      absorption: { low: 2.5, mid: 4.2, high: 6.0, conf: 82 },
      timeline: { low: 18, mid: 24, high: 36, conf: 75 },
      irr: { low: 14, mid: 22, high: 31, conf: 79 },
      margin: { low: 12, mid: 19, high: 27, conf: 84 },
    });
    setPredicting(false);
  };
...
function Sidebar({ active, setActive, collapsed, setCollapsed }) {
  ...
  return (
    <div style={{ ...S.side, width: collapsed ? 64 : 218, transition: "width 0.3s cubic-bezier(0.4, 0, 0.2, 1)", position: "relative" }}>
      ...
      <div style={{ position: "absolute", bottom: 60, right: collapsed ? 24 : 16, cursor: "pointer", color: C.gold, fontSize: 18, transition: "right 0.3s" }} onClick={() => setCollapsed(!collapsed)}>
        {collapsed ? "»" : "«"}
      </div>
...
function DataExplorerModal({ data, onClose }) {
  ...
  return (
    <div style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.8)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, backdropFilter: "blur(4px)" }} onClick={onClose}>
      <div style={{ background: C.bg3, border: `1px solid ${C.border}`, borderRadius: 8, padding: 24, width: 500, maxWidth: "90%", boxShadow: `0 20px 60px rgba(0,0,0,0.5)`, position: "relative" }} onClick={e => e.stopPropagation()}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, borderBottom: `1px solid ${C.border}`, paddingBottom: 12 }}>
          <div style={{ fontSize: 16, fontWeight: 600, color: C.gold, letterSpacing: 1, textTransform: "uppercase" }}>Data Node Inspector</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10, maxHeight: "60vh", overflowY: "auto" }}>
          {Object.entries(pd).map(([k, v], i) => {
            ...
            return (
              <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", background: i % 2 === 0 ? C.bg2 : "transparent", borderRadius: 4 }}>
                <span style={{ color: C.sub, fontSize: 13, textTransform: "capitalize" }}>{k.replace(/_/g, " ")}</span>
                <span style={{ color: C.text, fontSize: 13, fontWeight: 600 }}>{typeof v === 'number' ? (v > 1000 ? fmt.usd(v) : v) : String(v)}</span>
              </div>
            );
          })}
        </div>
        <div style={{ marginTop: 24, display: "flex", justifyContent: "flex-end" }}>
          <button style={S.btn("gold")} onClick={onClose}>Close Inspector</button>
        </div>
      </div>
    </div>
  );
